...
For scoped names (e.g., `@org/my-app`), use the unscoped portion (`my-app`) in memory instructions.
...
Create a new file with a header and the Headkey section:
...
### CLAUDE.md exists, no Headkey section

Append the Headkey section at the end of the file, separated by a blank line.

### CLAUDE.md exists, has `## Headkey Memory` section

Replace the existing section content. The section spans from `## Headkey Memory` to the next `##` heading or end of file. Preserve everything else in the file.

### CLAUDE.md exists, has Headkey content under a different heading

Ask the user whether to:
1. Replace the existing section with a standardized `## Headkey Memory` heading
2. Keep the existing heading and update content underneath
3. Add a separate `## Headkey Memory` section (not recommended — may cause conflicts)
